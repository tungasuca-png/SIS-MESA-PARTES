...
// Sin filtro por expediente_id: el backend ya acota este listado según el
// rol (personal interno ve todo; un SOLICITANTE ve solo lo que él mismo
// subió — ver getDocumentos en documentosService.js). Por eso esta pantalla
// no tiene un buscador libre por expediente, a diferencia de lo que tenía
// el módulo Documentos anterior: para un SOLICITANTE no hace falta, y no es
// su rol buscar en expedientes de otros.
...
function MisDocumentos() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            setError("");
            try {
                const data = await getDocumentos({ tipoDocumento, page, pageSize: DEFAULT_PAGE_SIZE });
                if (!active) return;
        ...
        <RoleLayout title="Documentos">
            <section className="dp-panel">
                ...
                <div className="dp-filters">
                    <select value={tipoDocumento} onChange={handleFilterChange} aria-label="Filtrar por tipo de documento">
                ...
                {!loading && !error && items.length === 0 && (
                    <p className="dp-table-empty">Aún no tienes documentos registrados.</p>
                )}
