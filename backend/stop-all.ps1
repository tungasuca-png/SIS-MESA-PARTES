# Baja los 6 procesos que levanta run-all.ps1.
#
...
$names = @("auth", "expedientes", "usuarios", "documentos", "derivaciones", "gateway")

$found = Get-Process -Name $names -ErrorAction SilentlyContinue
if (-not $found) {
    Write-Host "No hay procesos del backend corriendo (auth/expedientes/usuarios/documentos/derivaciones/gateway)." -ForegroundColor Yellow
    exit 0
}
